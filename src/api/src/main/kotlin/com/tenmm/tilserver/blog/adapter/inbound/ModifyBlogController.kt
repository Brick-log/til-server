...
class ModifyBlogController(
    ...
    @Operation(
        summary = "나의 블로그 수정",
        responses = [
            ApiResponse(responseCode = "200", description = "나의 블로그 수정 성공"),
            ApiResponse(responseCode = "400", description = "잘못된 블로그 수정 요청 (ex.잘못된 blog id)", content = [Content(schema = Schema(hidden = true))]),
